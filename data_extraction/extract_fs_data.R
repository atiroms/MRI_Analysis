...
path_in<-c("Dropbox/MRI/pnTTC/Puberty/Stats/T1w_FS/longitudinal/00_source/w1/13_meas",
           "Dropbox/MRI/pnTTC/Puberty/Stats/T1w_FS/longitudinal/00_source/w2/18_meas")
path_out<-"Dropbox/MRI/pnTTC/Puberty/Stats/T1w_FS/longitudinal/01_extract"
...
extract_fs<-function(paths_=paths){
  ...
  for (p_in in paths_$input){
    for (file_in in list.files(file.path(p_in,"output"))){
      ...
      df_out_sub<-cbind("ID_pnTTC"=df_out_sub$ID_pnTTC,"measure"=measure,"wave"=wave,df_out_sub[,-which(colnames(df_out_sub)=="ID_pnTTC")])
